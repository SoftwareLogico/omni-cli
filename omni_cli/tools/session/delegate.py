...
# `omni_cli.cli._run_task`, which writes `response.md` in the agent folder.
...
def execute_delegate_task(arguments: dict[str, Any], runtime: AppRuntime, parent_session_id: str) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["OMNI_SESSIONS_DIR"] = str(agents_dir)  # El hijo verá esta carpeta como su raíz de sesiones

    command = [
        sys.executable, "-m", "omni_cli",
