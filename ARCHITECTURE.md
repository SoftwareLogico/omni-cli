...
| --------------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `reasoning_effort`          | `openrouter`           | Hint for reasoning-class models. OpenRouter uses the nested format `"reasoning": {"effort": "<level>"}` — silently ignored on non-reasoning upstreams. Accepted values: `"none"`, `"minimal"`, `"low"`, `"medium"`, `"high"`, `"xhigh"`.                                        |
...
OpenRouter uses the unified nested object: `"reasoning": {"effort": "<level>"}`. The adapter encodes it accordingly when the user sets `reasoning_effort` in `[providers.openrouter]`. OpenRouter silently ignores it for non-reasoning upstreams, so it's safe to leave on by default. This nested format works with tools — unlike OpenAI's flat `reasoning_effort` field which Chat Completions rejects when tools are present.
